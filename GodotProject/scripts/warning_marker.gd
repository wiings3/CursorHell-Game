...
func apply_warning(warning: Dictionary, arena: Rect2) -> void:
	...
	# The CRT and new green arena were swallowing the old 2px / ~10% opacity guide.
	# Keep a dark separation line, a soft glow, and a bright core so telegraphs stay
	# instantly readable without changing the CRT shader itself.
	var guide_points := PackedVector2Array([Vector2(-11.0, 0.0), Vector2(guide_length - 11.0, 0.0)])
	guide_back.points = guide_points
	guide_glow.points = guide_points
	guide.points = guide_points
	guide_back.modulate.a = clampf(0.66 + 0.16 * pulse + 0.08 * sweep_focus, 0.0, 1.0)
	guide_glow.modulate.a = clampf(0.18 + 0.18 * pulse + 0.10 * sweep_focus, 0.0, 0.52)
	guide.modulate.a = clampf(0.58 + 0.34 * pulse + 0.08 * sweep_focus, 0.0, 1.0)
